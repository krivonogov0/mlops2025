...
## Реализован Мониторинг FastAPI-сервиса

### Запуск

1. **В первой консоли** — запустите сервис из шага 2:

   ```bash
   cd step2_fastapi_inference
   poetry run python main.py
   ```

2. **Во второй консоли** — запустите мониторинг:
...
Мониторинг будет проверять `/health` и `/predict`, писать цветные логи в консоль и сохранять метрики в `logs/`.

> Конфигурация: `config/monitoring_config.yaml`.
